...
/* 0xFF50 */	0x00, 0xFF, 0x80, 0xFF, 0xFF, 0xFF, 0xC2, 0x00,
/* 0xFF58 */	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
/* 0xFF60 */	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
/* 0xFF68 */	0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00,
/* 0xFF70 */	0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
uint8_t gbcc_ioreg_read(struct gbc *gbc, uint16_t addr, bool override)
{
	...
	if (addr == JOYP) {
		...
		if (check_bit(gbc->memory.ioreg[addr - IOREG_START], 5)) {
			joyp &= (uint8_t)~(uint8_t)(gbc->keys.start << 3u);
			joyp &= (uint8_t)~(uint8_t)(gbc->keys.select << 2u);
			joyp &= (uint8_t)~(uint8_t)(gbc->keys.b << 1u);
			joyp &= (uint8_t)~(uint8_t)(gbc->keys.a << 0u);
		}
		if (gbc->memory.ioreg[addr - IOREG_START] & (1u << 4u)) {
			joyp &= (uint8_t)~(uint8_t)(gbc->keys.dpad.down << 3u);
			joyp &= (uint8_t)~(uint8_t)(gbc->keys.dpad.up << 2u);
			joyp &= (uint8_t)~(uint8_t)(gbc->keys.dpad.left << 1u);
			joyp &= (uint8_t)~(uint8_t)(gbc->keys.dpad.right << 0u);
		}
		return joyp | (uint8_t)~mask;
	} 
	if (addr == NR52) {
		uint8_t tmp = 0;
		tmp = gbc->memory.ioreg[addr - IOREG_START];
		tmp |= (uint8_t)(gbc->apu.ch1.enabled << 0u);
		tmp |= (uint8_t)(gbc->apu.ch2.enabled << 1u);
		tmp |= (uint8_t)(gbc->apu.ch3.enabled << 2u);
		tmp |= (uint8_t)(gbc->apu.ch4.enabled << 3u);
		return  tmp;
	}
